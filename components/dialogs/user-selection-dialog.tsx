...
export const UserSelectionDialog: React.FC<UserSelectionDialogProps> = ({
    ...
    const users = (rawData?.data || []) as User[];
                    ...
                    {isLoading ? (
                        ...
                        filteredUsers.map((user) => (
                            <button
                                key={user.id}
                                onClick={() => {
                                    if (currentAssigneeId !== user.id) {
                                        onSelect(user.id);
                                        onClose();
                                    }
                                }}
                                disabled={currentAssigneeId === user.id}
                                className={cn(
                                    "w-full flex items-center gap-3 p-3 rounded-xl transition-all group",
                                    currentAssigneeId === user.id
                                        ? "bg-emerald-500/5 border border-emerald-500/20 cursor-not-allowed opacity-80"
                                        : "hover:bg-zinc-900"
                                )}
                            >
                                <div className={cn(
